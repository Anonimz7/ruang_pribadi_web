/* type-writing-2/type-coder.js — coder dari Natural Typewriter
 * Konversi dari tools/type-writing/coder.html ke module SPA. Dark mode di-scope ke halaman. */
...
const CSS = `
        ...
        twc-root.dark-mode #chat-container {
            background-color: #2a2a4a;
            box-shadow: 0 6px 18px rgba(0,0,0,0.4);
        }
         twc-root.dark-mode h2 {
            color: #9b86f0;
            border-color: #4f3a8c;
        }
         twc-root.dark-mode #response {
         ...
         twc-root.dark-mode #response code {
             color: #e0e0e0; /* Warna teks default di dark mode */
         }

         /* Dark mode untuk elemen select bahasa/tema di luar popup */
         twc-root.dark-mode .selection-group label {
             color: #e0e0e0;
         }
         twc-root.dark-mode .selection-group select {
             background-color: #2a2a4a;
             border-color: #4f3a8c;
             color: #e0e0e0;
         }
         twc-root.dark-mode .selection-group select:focus {
             border-color: #8a6fef;
             box-shadow: 0 0 0 2px rgba(138, 111, 239, 0.3);
         }


        twc-root.dark-mode #user-input {
            background-color: #2a2a4a;
            border-color: #4f3a8c;
            color: #e0e0e0;
        }
         twc-root.dark-mode #user-input::placeholder {
            color: #b0b0b0;
         }
         twc-root.dark-mode #user-input:focus {
            border-color: #8a6fef;
            box-shadow: 0 0 0 3px rgba(138, 111, 239, 0.3);
        }
         twc-root.dark-mode .cursor {
            background-color: #8a6fef;
        }
        twc-root.dark-mode .language-indicator,
        twc-root.dark-mode .typing-indicator {
            color: #b0b0b0;
        }
        twc-root.dark-mode #progress-bar-container {
            background-color: #4f3a8c;
         }
         twc-root.dark-mode #progress-bar {
             background-color: #8a6fef;
         }
         twc-root.dark-mode .button-group button:not(:disabled) {
             background-color: #8a6fef;
             color: #ffffff;
         }
          twc-root.dark-mode .button-group button:not(:disabled):hover {
             background-color: #7b5ae9;
         }
         twc-root.dark-mode button#stop-button:not(:disabled) {
             background-color: #e76f51;
         }
         twc-root.dark-mode button#stop-button:not(:disabled):hover {
             background-color: #d65f42;
         }
         twc-root.dark-mode button#pause-resume-button:not(:disabled) {
             background-color: #fca311;
             color: #212121;
         }
          twc-root.dark-mode button#pause-resume-button:not(:disabled):hover {
              background-color: #eb9300;
          }
         twc-root.dark-mode button:disabled {
             background-color: #555555;
             color: #b0b0b0;
         }

         /* Dark Mode Toggle Button Main Styling (inside popup) */
         twc-root.dark-mode #config-popup .dark-mode-toggle button { /* Added #config-popup */
            background-color: #fca311;
            color: #212121;
         }
          twc-root.dark-mode #config-popup .dark-mode-toggle button:hover:not(:disabled) { /* Added #config-popup */
            background-color: #eb9300;
         }


         /* Dark Mode Styling for Popup and Backdrop */
         twc-root.dark-mode #config-toggle-button {
            background-color: #8a6fef;
            color: #ffffff;
            box-shadow: 0 3px 10px rgba(0,0,0,0.3);
        }
         twc-root.dark-mode #config-toggle-button:hover {
            background-color: #7b5ae9;
         }

         twc-root.dark-mode #modal-backdrop {
             background-color: rgba(0, 0, 0, 0.85);
             backdrop-filter: blur(5px);
         }

        twc-root.dark-mode #config-popup {
            background-color: #2a2a4a;
            box-shadow: 0 12px 30px rgba(0,0,0,0.6);
            scrollbar-color: #4f3a8c #2a2a4a; /* Firefox dark mode scrollbar */
        }
         twc-root.dark-mode #config-popup::-webkit-scrollbar-track {
            background: #2a2a4a; /* Webkit dark mode scrollbar track */
         }
         twc-root.dark-mode #config-popup::-webkit-scrollbar-thumb {
            background: #8a6fef; /* Webkit dark mode scrollbar thumb */
         }
         twc-root.dark-mode #config-popup::-webkit-scrollbar-thumb:hover {
             background: #7b5ae9;
         }

         twc-root.dark-mode #config-popup h3 {
            color: #9b86f0;
            border-color: #4f3a8c;
        }
         twc-root.dark-mode #code-element-configs > div { /* Changed ID */
             background-color: #3a3a5a;
             border-color: #4f3a8c;
         }
         twc-root.dark-mode #code-element-configs label { /* Changed ID */
             color: #e0e0e0;
         }
          twc-root.dark-mode .speed-input-group label,
          twc-root.dark-mode .pause-input-group label {
              color: #b0b0b0;
          }
          twc-root.dark-mode #code-element-configs input[type="number"] { /* Changed ID */
             background-color: #2a2a4a;
             border-color: #555;
             color: #e0e0e0;
         }
         twc-root.dark-mode #config-popup p {
             color: #b0b0b0;
         }
         twc-root.dark-mode #config-popup .dark-mode-toggle {
             border-color: #4f3a8c;
         }


         /* Dark Mode Close Button Style */
         twc-root.dark-mode #config-popup .close-button {
             color: #b0b0b0;
         }
         twc-root.dark-mode #config-popup .close-button:hover {
            color: #e0e0e0;
         }
...
const BODY = `<div id="chat-container">
        ...
        <div class="controls-container">
         ...
         <div id="typing-status" class="typing-indicator">Mengetik kode...</div> <textarea id="user-input" placeholder="Tempel kode Anda di sini... (Mendukung berbagai bahasa pemrograman)" spellcheck="false"></textarea> <div class="button-group">
             <button id="cmd-startTypingCode">Mulai Mengetik Kode</button> <button id="pause-resume-button" id="cmd-togglePauseResumeCode" disabled>Jeda Pengetikan</button> <button id="stop-button" id="cmd-stopTypingCode" disabled>Hentikan Pengetikan</button> </div>

        <button id="config-toggle-button" id="cmd-toggleConfigPopup">
            <i class="fas fa-cog"></i> </button> <div id="modal-backdrop"></div>

        <div id="config-popup">
             <button class="close-button" id="cmd-toggleConfigPopup">
            ...
            <p style="font-size: 12px; color: #6c757d; margin-top: 0; margin-bottom: 15px; line-height: 1.4;">Sesuaikan nilai-nilai ini untuk mengubah kecepatan pengetikan per jenis elemen kode. Kecepatan Dasar adalah jeda per karakter. Pengali Jeda memengaruhi jeda setelah elemen.</p> <h4>Tampilan Website</h4> <div class="dark-mode-toggle">
                <button id="cmd-toggleDarkMode"><i class="fas fa-moon"></i> Mode Gelap</button> </div>
...
function initMode(page) {
        ...
        const responseElement = page.querySelector('#response');
        // Dapatkan elemen code di dalam response
        const codeElement = responseElement.querySelector('pre code');

        const typingIndicator = page.querySelector('#typing-status');
        const userInputElement = page.querySelector('#user-input');
        const stopButton = page.querySelector('#stop-button');
        const sendButton = page.querySelector('.button-group button:not(#stop-button):not(#pause-resume-button)');
        const pauseResumeButton = page.querySelector('#pause-resume-button');
        const progressBar = page.querySelector('#progress-bar');
        // Dapatkan elemen select bahasa dan tema syntax
        const languageSelectElement = page.querySelector('#language-select');
        const themeSelectElement = page.querySelector('#theme-select');
        // Dapatkan elemen link stylesheet Highlight.js utama
        const hljsThemeLinkElement = page.querySelector('#hljs-theme-main');


        const codeElementConfigsDiv = page.querySelector('#code-element-configs'); // Dapatkan div konfigurasi
        const configPopup = page.querySelector('#config-popup'); // Dapatkan elemen popup
        const modalBackdrop = page.querySelector('#modal-backdrop'); // Dapatkan elemen backdrop

        // Tombol toggle dark mode (berada di dalam popup sekarang)
        const darkModeToggleButton = page.querySelector('#config-popup .dark-mode-toggle button');
        ...
        */
        ...
        function toggleDarkMode() {
            ...
            const isDarkMode = page.classList.contains('dark-mode');
            if (isDarkMode) {
        ...
        function updateDarkModeButtonIcon() {
            const darkModeButton = page.querySelector('#config-popup .dark-mode-toggle button');
...
function bootstrap() {
             ...
             let mainThemeLink = page.querySelector('#hljs-theme-main');
        ...
        const languageDisplayElement = page.querySelector('#language-display');
  ...
  bootstrap();
    const el_startTypingCode = page.querySelector('#cmd-startTypingCode');
    if (el_startTypingCode) el_startTypingCode.addEventListener('click', startTypingCode);
    const el_togglePauseResumeCode = page.querySelector('#cmd-togglePauseResumeCode');
    if (el_togglePauseResumeCode) el_togglePauseResumeCode.addEventListener('click', togglePauseResumeCode);
    const el_stopTypingCode = page.querySelector('#cmd-stopTypingCode');
    if (el_stopTypingCode) el_stopTypingCode.addEventListener('click', stopTypingCode);
    const el_toggleConfigPopup = page.querySelector('#cmd-toggleConfigPopup');
    if (el_toggleConfigPopup) el_toggleConfigPopup.addEventListener('click', toggleConfigPopup);
    const el_toggleDarkMode = page.querySelector('#cmd-toggleDarkMode');
    if (el_toggleDarkMode) el_toggleDarkMode.addEventListener('click', toggleDarkMode);

}

export function render() {
  ensureDeps();
  const page = createEl('div', { class: 'twc-root' });
