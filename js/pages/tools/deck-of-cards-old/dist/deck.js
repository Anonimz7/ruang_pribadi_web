...
var Deck = (function () {
  ...
  function _card(i, options) {
    var transform = prefix('transform');
    var settings = options || {};
    var isCustom = settings.custom === true;
    var label = settings.label == null ? '' : settings.label;
    ...
    if (isCustom) {
      var normalized = normalizeCustomLabel(label);
      var parts = normalized.parts;

      if (parts.length > 1) {
        parts.forEach(function (part, index) {
          var $segment = createElement('div');
          $segment.classList.add(index === 0 ? 'label-term' : 'label-translation');
          $segment.textContent = part;
          $label.appendChild($segment);

          if (index < parts.length - 1) {
            var $divider = createElement('div');
            $divider.classList.add('label-divider');
    ...
    function normalizeCustomLabel(rawLabel) {
      if (rawLabel == null) {
        return { parts: [''] };
      }

      if (typeof rawLabel === 'object') {
        if (Array.isArray(rawLabel.parts)) {
          var objectParts = normalizeLabelParts(rawLabel.parts);
          if (objectParts.length > 0) {
            return { parts: objectParts };
          }
        }

        var term = readFirstValue(rawLabel, ['term', 'word', 'front', 'text', 'label']);
        var translation = readFirstValue(rawLabel, ['translation', 'meaning', 'back', 'definition']);
        ...
        if (translation) {
          return { parts: normalizeLabelParts([term, translation]) };
        }

        return { parts: normalizeLabelParts([term]) };
      }

      var labelText = String(rawLabel);
      var parts = normalizeLabelParts(labelText.split('='));

      if (parts.length > 1) {
        return { parts: parts };
      }

      return { parts: normalizeLabelParts([labelText]) };
    }

    function normalizeLabelParts(rawParts) {
    ...
    function readFirstValue(object, keys) {
      for (var i = 0; i < keys.length; i++) {
        if (object[keys[i]] != null && object[keys[i]] !== '') {
    ...
    card: function card(_card3) {
      ...
      _card3.shuffle = function (cb) {
        ...
        _card3.animateTo({
          ...
          x: plusminus(Math.random() * 50 + 25) * ____fontSize / 16,
          y: -z,
          rot: 0
        });
        _card3.animateTo({
    ...
    card: function card(_card4) {
      ...
      _card4.poker = function (i, len, cb) {
        ...
        _card4.animateTo({
          ...
          x: Math.round((i - 2.05) * 88 * __fontSize / 16),
          y: Math.round(-138 * __fontSize / 16),
          rot: 0,
    ...
    card: function card(_card6) {
      ...
      _card6.fan = function (i, len, cb) {
        ...
        _card6.animateTo({
          ...
          x: Math.cos(deg2rad(rot - 90)) * 69 * _fontSize / 16,
          y: Math.sin(deg2rad(rot - 90)) * 69 * _fontSize / 16,
          rot: rot,
    ...
    card: function card(_card7) {
      ...
      _card7.bysuit = function (cb) {
        ...
        _card7.animateTo({
          ...
          x: -Math.round((6.75 - rank) * 10 * ___fontSize / 16),
          y: -Math.round((1.5 - suit) * 115 * ___fontSize / 16),
          rot: 0,
  ...
  function Deck(options) {
    ...
    // init cards array
    var cards = new Array(settings.cardCount);
    ...
    for (var i = cards.length; i; i--) {
      card = cards[i - 1] = _card(i - 1, {
        custom: settings.useCustom,
        label: settings.customData[i - 1]
      });
      card.setSide('back');
      card.mount($el);
    }
  ...
  function normalizeOptions(options) {
    if (options && typeof options === 'object') {
      var customData = Array.isArray(options.data) ? options.data.slice(0, 52) : [];
      var useCustom = options.custom === true || options.mode === 'custom' || customData.length > 0;
      return {
        cardCount: useCustom ? customData.length : options.jokers ? 55 : 52,
        useCustom: useCustom,
